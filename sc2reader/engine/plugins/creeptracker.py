    ...
    def handleEndGame(self, event, replay):
        ...
        for player in replay.players:
            if player.creep_spread_by_minute:
                player.max_creep_spread  =\
                    max(player.creep_spread_by_minute.items(),
                        key=lambda x: x[1])
            else:
...
class creep_tracker():
    def __init__(self, replay):
        ...
        self.unit_name_to_radius = {"CreepTumor": 10,
                                    "Hatchery": 8,
                                    "NydusCanal": 5}
        ...
        cropsizeX = replay.map.map_info.camera_right\
            - replay.map.map_info.camera_left
        cropsizeY = replay.map.map_info.camera_top\
        ...
        self.map_width = int(cropsize[0] *
                             (float(self.map_height) / cropsize[1]))
        ...
        mapCenter = [mapOffsetX + cropsize[0] / 2.0,
                     mapOffsetY + cropsize[1] / 2.0]
    ...
    def init_cgu_lists(self, player_id):
        ...
        if self.debug:
            self.creep_spread_image_by_minute[player_id] =\
                defaultdict(StringIO)
        self.creep_gen_units[player_id] = list()
        self.creep_gen_units_seconds[player_id] = list()

    def add_to_list(self, player_id, unit_id, unit_location,
                    unit_type, event_time):
    ...
    # Each time frame contains a list of all CGUs that are alive
        ...
        if length_cgu_list == 0:
            ...
            previous_list =\
                self.creep_gen_units[player_id][length_cgu_list - 1][:]
    ...
    ## and appended as a new time frame
        for player_id in self.creep_gen_units:
            ...
            cgu_per_player =\
                self.creep_gen_units[player_id][length_cgu_list - 1]
            creep_generating_died =\
                filter(lambda x: x[0] == unit_id, cgu_per_player)
            for creep_generating_died_unit in creep_generating_died:
                new_cgu_per_player = list(filter(
                    lambda x: x != creep_generating_died_unit, cgu_per_player))
    ...
    ## this function returns the index and value of every cgu time
        ...
        for i in range(0, maximum_cgu_time):
            a = list(filter(
                lambda x_y: x_y[1] // 60 == i, enumerate(cgu_time_list)))
            if len(a) > 0:
                yield a
    ...
    #the most cgus every minute will be used to represent that minute
        if len(self.creep_gen_units_seconds[player_id]) == 0:
            return
        cgu_per_minute1, cgu_per_minute2 =\
            tee(self.cgu_gen_times_to_chunks(
                self.creep_gen_units_seconds[player_id]))
        cgu_unit_max_per_minute =\
            self.cgu_in_min_to_cgu_units(player_id, cgu_per_minute1)
    ...
    ## Iterates through all cgus and and calculate the area
        for index, cgu_per_player in enumerate(
                self.creep_gen_units[player_id]):
            ...
            if self.debug:
                cgu_event_time_str = str(int(cgu_event_seconds // 60)) +\
                    ":" + str(cgu_event_seconds % 60)
                self.print_image(creep_area_positions, player_id,
                                 cgu_event_time_str)
            creep_area = len(creep_area_positions)
            self.creep_spread_by_minute[player_id][cgu_event_seconds] =\
    ...
    def print_image(self, total_points_on_map, player_id, time_stamp):
        ...
        self.creep_spread_image_by_minute[player_id][time_stamp] =\
            creeped_imageIO
    ...
    def check_image_pixel_within_boundary(self, pointX, pointY):
        ...
        pointX =\
            int(self.map_width - 1 if pointX >= self.map_width else pointX)
        pointY =\
            int(self.map_height - 1 if pointY >= self.map_height else pointY)
