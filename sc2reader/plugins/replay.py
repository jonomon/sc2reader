...
from sc2reader.plugins.creeptracker import creep_tracker
...
def CreepTracker(replay):
    ...
    for event in replay.events:
        creepTracker.add_event(event)
    ##remove all but one time frame per minute
    for player_id in replay.player:
        creepTracker.reduce_cgu_per_minute(player_id)
    ##Analyse the creep spread area
    for player in replay.players:
        player.creep_spread_by_minute = creepTracker.get_creep_spread_area(player.pid)
    ## get the maximum creep spread and time
    for player in replay.players:
